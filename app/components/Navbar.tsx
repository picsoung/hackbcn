...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const eventSlugFromPath = pathParts.length >= 3 ? pathParts[2] : null
  ...
  return (
    <nav className={`${theme.colors.navbar}`}>
      <Disclosure as="div">
        {({ open }) => {
                  ...
                  <div className="hidden sm:ml-6 sm:flex sm:space-x-5">
                    {navigationOptions.map((option) => (
                      <Link
                        key={option.name}
                        href={option.href}
                        className={`inline-flex items-center text-lg font-medium ${theme.colors.text} hover:text-opacity-80`}
                      >
                        <p>{option.name}</p>
                      </Link>
                    ))}
