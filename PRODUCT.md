...
1. **Photographic, polaroid-coded signature motif.** The polaroid stack is the brand's strongest distinctive move. Lean in: hand-rotated photos, tape labels, captions in a real voice, asymmetric placement. Polaroids appear across the brand — homepage hero, event recaps, mentor/judge introductions, hack night galleries. They differentiate from the SaaS-community-landing template by being unmistakably hand-placed.
...
4. **One committed accent, deliberate seconds.** `org-accent` coral is the parent brand colour. Second colours appear only when they carry a deliberate role (e.g. a sub-event's micro-identity). Stray slate/teal/purple leftovers are the template fingerprint.
